...
public class Simulator {
	...
	private void creaEventi() {
		...
		for (int i = 0; i < this.NUM_EVENTI; i++) {
			...
			Duration durata = Duration
					.ofMinutes(this.DURATA_MIN + (int) (Math.random() * (this.DURATA_MAX - this.DURATA_MIN + 1)));
	...
	private void processaEvento(Event e) {
		...
		case ARRIVO_GRUPPO_CLIENTI:
			...
			for (Tavolo t : this.tavoli) {
				if (!t.isOccupato() && t.getPosti() >= e.getnPersone()
						&& (t.getPosti() * OCCUPAZIONE_MAX) <= e.getnPersone()) {
					tavolo = t;
					break;
				}
			}
			if (tavolo != null) {
				...
				e.setTavolo(tavolo);
				queue.add(new Event(EventType.TAVOLO_LIBERATO, e.getTime().plus(e.getDurata()), e.getnPersone(),
				...
				if (bancone <= e.getTolleranza()) {
					...
					System.out.format("%d personensi vanno a casa\n", e.getnPersone());
		...
		case TAVOLO_LIBERATO:
			System.out.println("Tavolo liberato da: " + e.getTavolo().getPosti());
